...
int ec_config::writevalue( const char *psec, const char *pkey, const char *pvalue, const char *ppath )
{
	...
	{
		for( int iindex_sec = 0; iindex_sec < cfg_file.cfg_secs.size(); iindex_sec++ )
		{
			if( -1 != cfg_file.cfg_secs[iindex_sec].ssec.find(psec) )
			{
				ipos_sec = iindex_sec;
			...
			if( cfg_file.cfg_secs[iindex_sec].cfg_infos.size() > 0 )
			{
					...
					iindex_key < cfg_file.cfg_secs[iindex_sec].cfg_infos.size(); iindex_key++ )
				{
					...
					{
						if( -1 != ipos_sec &&
							-1 != cfg_file.cfg_secs[iindex_sec].cfg_infos[iindex_key].skeyname.find(pkey) )
						{
							ipos_key = iindex_key;
							break;
						}
