...
void* MonitorSoapServerMain(void *port)
{
	...
	int result = soap_bind(&csoapExtern, NULL, *(int *)port, MaxClientAmount);
	cout << *(int *)port <<endl;
	if(result < 0)
	{
...
int MonitorForBrowser(struct soap *soapHandle)
{
	...
	ifstream fin("./webservice/mons.wsdl", ios::binary);

	//ifstream fin("nvr.wsdl", ios::binary);
	if(!fin.is_open())
	{
		return 404;
	}
